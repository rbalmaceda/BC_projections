...
aggregate_djf_Ind35 <- function(grid) {
  pp=indexGrid(tx = grid, time.resolution = "year", index.code = "TXth", th=35)
  pp$Dates <- grid$Dates
  return(pp)
}
...
for (method in methods) {
  ...
  if (special_case) {
    ...
    swbgt_train <- map(1:length(grids_train), function(g) {
      ...
      aggregate_djf(calc_swbgt(Tx, Hus))
    ...
    swbgt_train <- map2(tasmax_train, hus_train, ~ {
      aggregate_djf(calc_swbgt(.x, .y))
    }) %>% bindGrid(dimension = "member")
  }
  ...
  if (special_case) {
    ...
    swbgt_fut <- map2(tasmax_fut, hus_fut, ~ {
      aggregate_djf(calc_swbgt(.x, .y))%>% 
        subsetGrid(years = 2006:2098)
    }) %>% bindGrid(dimension = "member")
  }
...
save(list = ls(pattern = "^swbgt_BC_serie_list_"), file = paste0("swbgt_BC_serie_list_",RCMs[k],".rda"))
...
swbgt_raw_hist <- map2(tasmax_hist, hus_hist, ~ {
  aggregate_djf(calc_swbgt(.x, .y))
}) %>% bindGrid(dimension = "member")
...
swbgt_raw_fut <- map2(tasmax_fut, hus_fut, ~ {
  aggregate_djf(calc_swbgt(.x, .y))%>% 
    subsetGrid(years = 2006:2098)
}) %>% bindGrid(dimension = "member")
...
assign("swbgt_raw_serie_list", raw_serie_list)
save(list = ls(pattern = "^swbgt_raw_serie_list"), file = paste0("swbgt_raw_serie_list_",RCMs[k],".rda"))
