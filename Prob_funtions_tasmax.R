...
tasmax_raw_train <- map(files_train_raw, function(f) {
...
# names(tasmax_raw_train) <- name[1:3]
# names(tasmax_raw_train) <- name[7:9]
tasmax_raw_df <-rbind(bind_rows(tasmax_raw_train[[1]]),bind_rows(tasmax_raw_train[[2]]),bind_rows(tasmax_raw_train[[3]]))
row.names(tasmax_raw_df)=NULL
tasmax_raw_df$modelo=rep(name[7:9],each=99)
tasmax_raw_df$method=rep("train",297)


plot(tasmax_raw_train[[2]]$percentil, tasmax_raw_train[[2]]$dif, type="l",ylim=c(-3,3),
     xlab="Percentil", ylab="Obs - RAW")
abline(h=0, lty=2)
lines(tasmax_raw_train[[1]]$dif,col='red')
lines(tasmax_raw_train[[3]]$dif,col='green')
  ...
  for (j in seq_along(methods)) {
    ...
    special_case <- method %in% c("mbcr", "mbcn")
     ...
     files_fut_method <- list.files(
      ...
      pattern = paste0("tasmax.*.", method, ".*._RCP8.5_SESA.rda"), 
      full.names = TRUE
    )
    ...
    if (special_case) {
      BC <- subsetGrid(BC, var = BC[["Variable"]][["varName"]][1])
    }
    # --- Alinear temporalmente ---
...
pp_all_all=rbind(pp_all,tasmax_raw_df)
...
ggsave(p,filename=paste0(var,"_P_fut_punto_",rcms[k],"_p2.png"),device="png",dpi=300,width=6,height = 4,scale=0.7)
