...
for (method in methods) {
  ...
  if (special_case) {
    ...
    swbgt_train <- map(1:length(grids_train), function(g) {
      ...
      if(index=='Ind30'){
       out <- calc_swbgt(Tx, Hus) %>% aggregate_djf_Ind30()
       out$Dates <- aggregate_djf(Tx)$Dates  }
      if(index=='max'){
      out <- calc_swbgt(Tx, Hus) %>% aggregate_djf_max()}
        return(out)
    ...
    swbgt_train <- map2(tasmax_train, hus_train, function(tx, hu) {
      if(index=='Ind30'){
      out <- calc_swbgt(tx, hu) %>% aggregate_djf_Ind30()
      out$Dates <- aggregate_djf(tx)$Dates
      }
      if(index=='max'){
      out <- calc_swbgt(tx, hu) %>% aggregate_djf_max()
      }
      return(out)
    }) %>% bindGrid(dimension = "member")
    
  } 
  swbgt_train$Members <- members_name[list_rcms[[k]]]
  swbgt_train[["Variable"]][["varName"]] <- "swbgt"
  
  swbgt_train_esc <- scaleGrid(
    grid = swbgt_train,
    base = subsetGrid(swbgt_train, years = 1984:2003),
    by.member = TRUE
  )
  ...
  if (special_case) {
    ...
    swbgt_fut <- map(1:length(grids_fut), function(g) {
      ...
      if(index=='Ind30'){
      out <-aggregate_djf_Ind30(calc_swbgt(Tx, Hus))
      out$Dates <- aggregate_djf(Tx)$Dates }
      if(index=='max'){
        out <-aggregate_djf_max(calc_swbgt(Tx, Hus))}
  ...
  swbgt_fut <- map2(tasmax_fut, hus_fut, function(tx, hu) {
    if(index=='Ind30'){
    out <- calc_swbgt(tx, hu) %>% 
      aggregate_djf_Ind30()
    out$Dates <- aggregate_djf(tx)$Dates  }
    if(index=='max'){
      out <- calc_swbgt(tx, hu) %>% 
  ...
  swbgt_fut$Members <- members_name[list_rcms[[k]]]
  
  swbgt_fut_esc <- scaleGrid(
    grid = swbgt_fut,
    base = subsetGrid(swbgt_train, years = 1984:2003),
    by.member = TRUE
  )
  ...
  # --- Guardo la lista ---
  BC_serie_list <- list(
    swbgt_BC_train     = swbgt_train,
    swbgt_BC_train_esc = swbgt_train_esc,
    swbgt_BC_fut       = swbgt_fut,
    swbgt_BC_fut_esc   = swbgt_fut_esc
  )
  
  assign(paste0("swbgt_BC_serie_list_", method), BC_serie_list)
}

save(list = ls(pattern = "^swbgt_BC_serie_list_"), file = paste0("swbgt_",index,"_BC_serie_list_",RCMs[k],".rda"))
...
swbgt_raw_hist <- map2(tasmax_hist, hus_hist, function(tx, hu) {

  if(index=='max'){
    out=aggregate_djf_max(calc_swbgt(tx, hu)) %>% 
      subsetGrid(years = 1981:2004)
  }
  if (index=='Ind30'){
  out=aggregate_djf_Ind30(calc_swbgt(tx,hu)) %>%
...
swbgt_raw_hist$Members <- members_name[list_rcms[[k]]]
swbgt_raw_hist[["Variable"]][["varName"]] <- "swbgt"

if(index=='max'){
  swbgt_hist_esc <- scaleGrid(
  grid = swbgt_raw_hist,
  base = subsetGrid(swbgt_raw_hist, years = 1984:2003),
  # base = subsetGrid(swbgt_raw_hist, years = 1986:2005),
...
if(index=='Ind30'){
  swbgt_hist_esc <- scaleGrid(
    grid = swbgt_raw_hist,
    base = subsetGrid(swbgt_raw_hist, years = 1985:2004),
    # base = subsetGrid(swbgt_raw_hist, years = 1986:2005),
...
swbgt_raw_fut <- map2(tasmax_fut, hus_fut, function(tx, hu) {
  if(index=='max'){
   out= aggregate_djf_max(calc_swbgt(tx, hu))%>% 
...
swbgt_raw_fut$Members <-members_name[list_rcms[[k]]]

if(index=='max'){
swbgt_raw_fut_esc <- scaleGrid(
  grid = swbgt_raw_fut,
  base = subsetGrid(swbgt_raw_hist, years = 1984:2003),
  # base = subsetGrid(swbgt_raw_hist, years = 1986:2005),
  by.member = TRUE
...
raw_serie_list <- list(
  swbgt_raw_train     = swbgt_raw_hist,
  swbgt_raw_train_esc = swbgt_hist_esc,
  swbgt_raw_fut       = swbgt_raw_fut,
  swbgt_raw_fut_esc   = swbgt_raw_fut_esc
)

assign("swbgt_indice_raw_serie_list", raw_serie_list)
save(list = ls(pattern = "^swbgt_indice_raw_serie_list"), file = paste0("swbgt_",index,"_raw_serie_list_",RCMs[k],".rda"))
